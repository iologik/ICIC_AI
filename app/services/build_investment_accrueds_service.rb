...
class BuildInvestmentAccruedsService < BaseService
  MOVE_DOWN_POINT = 7

  # rubocop:disable Metrics/AbcSize
  def call(investment_accrueds)
    tb_data = payment_data(investment_accrueds)
  ...
  def payment_data(investment_accrueds)
    payment_result = [['Investment', 'Accrued CAD', 'Accrued USD']]
    total_amount = 0
    investment_accrueds.each do |invest|
      total_amount += invest.accrued
    end

    cad_accrued = usd_accrued = 0
    investment_accrueds.each do |invest|
      if invest.currency == 'CAD'
        cad_accrued += invest.accrued
        payment_result << [invest.name,
                           number_to_currency(invest.accrued), nil]
      else
        usd_accrued += invest.accrued
        payment_result << [invest.name,
                           nil, number_to_currency(invest.accrued)]
      end
    end

    payment_result << ['Total', number_to_currency(cad_accrued), number_to_currency(usd_accrued)]
  end
