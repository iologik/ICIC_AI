...
#
class InvestmentRetained < Investment
  def retained
    @retained ||= sub_investments.inject(0) do |r, invest|
      r += invest.current_retained
      r
    end
  end

  def retained_by_date(date)
    @retained_by_date ||= sub_investments.inject(0) do |r, invest|
      r += invest.retained_by_date(date)
      r
    end
  end

  def retained_until_this_year
    @retained_until_this_year ||= retained_by_date((Time.zone.today - 1.year).at_end_of_year)
  end

  def self.with_retained
    investments = []
    InvestmentRetained.find_each do |investment|
      investments << investment if investment.retained.positive?
    end
